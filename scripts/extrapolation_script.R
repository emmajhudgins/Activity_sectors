### Script for extrapolating economic burden of invasive alien species to primary sectors
...
### Import sTwist alien species range/first record database ###
stwist<-read.table('./data/SInAS_AlienSpeciesDB_2.4.1.csv', header=T)
...
stwist_old<-read.table('./data/sTwist_database.csv', header=T)
...
#import CABI data
cabi_categories<-read.csv('./data/cabi_by_sector_v2.csv')
cabi_categories$Cum.Cost<-cabi_categories$Cum.Cost<-0

#merge with invacost data
...
#import taxonomic groupings 
taxonomy<-read_xlsx('./data/species_coarse_categories_IC4_1.xlsx')
...
#subset to sectors of interest and output ranking
...
#look at proportional breakdown by group
bygroup<-aggregate(invacost_agg$Cum.Cost, by=list(invacost_agg$tax, invacost_agg$Impacted_sector_2), sum)
...
## Extrapolation - create fitting dataset
...
#take minimum first record date when multiple exist
...
#calculate number of independent introductions
...
# fit global boosted regression tree
...
#write.csv(as.data.frame(summary(brt_glob)), './output/brt_glob.csv', row.names=F)

## sector-specific extrapolation 

sector="Fishery" #change this to sector of interest
...
# create data frame of 'missing' species data
...
#write.csv(data_pred, file=paste0("./output/data_pred_3", sector, ".csv"), row.names=F)


#get continent-specific cost data
...
#calculate total cost by continent and cost type
...
#write.csv(allcosts, file=paste0("./output/predicted_costs_3", sector,".csv")) 
...
sum(exp(predict.gbm(brt_glob,newdata=subset(data_pred, Species%in%invacost_agg$Species==F), n.trees = brt_glob$n.trees))/1000000)+sum(invacost_sub_agg$Cum.Cost/1000000)
 ...
 # brt (attribute-based scenario) by cost type
...
#total observed cost by species
...
ggplot(data) + geom_ribbon(data=data2, aes(x=x, ymin=colmin, ymax=colmax),fill=viridis(5)[2], alpha=0.5)+ scale_x_continuous(name="Annual Cost (million US$)",labels = function(x) format(x, scientific = TRUE))+scale_y_continuous(name="Probability Density")+ geom_line(data=data2,aes(y=colmid,x=x))+theme_classic()+geom_point(data=data.frame(y),aes(x=y), y=colmid[points], colour=viridis(1)[1])+theme(axis.text=element_text(size=11))+theme(plot.margin=unit(c(0.5,1,0.5,0.5), "cm")) # plots distribution of annual costs across species in millions USD, observations of invacost costs per species as points


#sample from posterior distribution of total costin millions
